...
export const AWS_IOT_PROVISION_TOPICS = {
  ...
  registerThingAccepted: (templateName: string) =>
    `$aws/provisioning-templates/${templateName}/provision/json/accepted`,
  registerThingRejected: (templateName: string) =>
...
export async function execProvision(argv: Args): Promise<void> {
  ...
  try {
    ...
    const { thingResponse } = await execute_register_thing(
      provisionDevice,
      keysAndCertificateResponse.certificateOwnershipToken!,
      argv,
    );
...
async function execute_register_thing(
  provisionDevice: device,
  certificateOwnershipToken: string,
  argv: Args,
): Promise<IExecRegisterThingResponse> {
  ...
  return new Promise(async (resolveDone, rejectDone) => {
    ...
    await new Promise((resolve: any, reject: any) => {
      provisionDevice.subscribe(
        AWS_IOT_PROVISION_TOPICS.registerThingAccepted(argv.templateName),
        { qos: 1 },
        (err) => {
          if (err) {
            reject(err);
    ...
    await new Promise((resolve: any, reject: any) => {
      provisionDevice.subscribe(
        AWS_IOT_PROVISION_TOPICS.registerThingRejected(argv.templateName),
        { qos: 1 },
        (err) => {
          if (err) {
            reject(err);
    ...
    await new Promise((resolve, reject) => {
      provisionDevice.publish(
        AWS_IOT_PROVISION_TOPICS.registerThing(registerThingRequestPayload.templateName),
        JSON.stringify(registerThingRequestPayload),
        { qos: 1 },
        (err) => {
          if (err) {
            reject(err);
